...
type APIResponse<T> = {
  data: T;
  msg: string | null;
  code: import("./constants").APIResponseCode;
};
...
type Member = {
  ...
  country: string;
  gender: number;
  status: import("./constants").MemberStatus;
...
type MemberMemberGetSettingResponse = {
  ...
  unitType: import("./constants").GrainUnit;
  enableBiometric: boolean;
...
type FeedingPlanToday = {
  ...
  time: string;
  grainNum: number;
  state: import("./constants").PlanState;
  repeat: boolean;
};
...
type SharedDevice = {
  ...
  type: import("./constants").ShareType;
  shareWay: import("./constants").SharedDeviceState;
  state: number;
  recordState: number;
};
